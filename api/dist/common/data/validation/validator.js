...
class Validator {
    ...
    buildSchema() {
        return new Promise((resolve, reject) => {
            ...
            let pLength = this.entityType.props.length;
            let promises = new Array(pLength);
            for (let idx = 0; idx < pLength; idx++) {
                let prop = this.entityType.props[idx];
            ...
            if (validation.type === models_1.PropertyType.linkedEntity) {
                ...
                if (definitions && definitions[validation.ref.name])
                    ...
                    entity_repository_1.EntityRepository.getRepositoty(models_1.SysEntities.entityType).then(etRepo => {
                        ...
                        let promises = new Array(absPropsLength);
    ...
    validateLinkedEntities() {
        return new Promise((resolve, reject) => {
            let problems = [];
            let propsLength = this.entityType.props.length;
            let promises = new Array(propsLength);
