...
import { IEntityType, IValidation, SysEntities } from "@poseidon/core-models";
...
export class AbstractEntitySchamBuilder implements ISchamaBuilderStrategy {
  ...
  async build(rootSchema: FluentSchemaBuilder, validation: IValidation): Promise<FluentSchemaBuilder> {
    ...
    if (definitions && definitions[ref.name]) return new FluentSchemaBuilder({}).$ref("#/definitions/" + ref.name);
    // If the root schema doesn't have a definition for this entity type, we build one.
      ...
      const queryResult = await this.context.executeQuery(SysEntities.entityType, { _id: ref._id });
      const abstractEtType = queryResult.result.data[0];
